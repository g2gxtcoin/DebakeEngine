...
use std::{collections::btree_map::IntoKeys, io::Read, path::PathBuf};
...
use input::{
    env::{InputE, InputKey1D, InputKey2D, InputUnit1D, InputUnit2D, InputUnitT, InputValue1D},
    win::env::WinInputE,
};
use manager::{
    buffer::env::Buffers,
...
fn main() -> std::io::Result<()> {
    ...
    unsafe {
        ...
        exe.input = InputE::build()
            .build_active_keys(input::env::cfg::DEFAULT_4X_1D.to_vec(), 1)
            .build_active_keys(input::env::cfg::DEFAULT_4X_2D.to_vec(), 2);
    }
    ...
    exe.render_cmd1 = exe
        ...
        .build_sync(&exe.renderer1, true)
        .build_usage(CmdUsage::PIPE_GRAPHIC | CmdUsage::MANUAL_MODE)
    ...
    // CBO
    exe.renderer1
        .tak_create_cmd_buffer(tak.render_task.get_data_mut(exe.renderer1.id).unwrap());
    ...
    exe.render_cmd1
        .tak_init_submit_info(tak.rendercmd_task.get_data_mut(exe.render_cmd1.id).unwrap());
    ...
    exe.render_cmd1
        .tak_begin_render_pass(tak.rendercmd_task.get_data_mut(exe.render_cmd1.id).unwrap());
    ...
    exe.render_cmd1
        .reset_cmd(dat.cmd_buf.get_data_mut(exe.renderer1.id).unwrap());
    ...
    ////////////////////////////////////////////////////////
    ////                                                ////
    ////            RENDERER SURFACE COLLECT            ////
    ////                                                ////
    ////////////////////////////////////////////////////////

    exe.renderer1.present_surfimg_color(&mut exe.render_cmd1);
    ...
    while unsafe { workarea::WORKAREA_CLOSE == false } {
        ...
        std::thread::sleep(std::time::Duration::new(0, 0010000_000));
        exe.render_cmd1.wait_fences(&exe.timer);
...
struct DatumM {
    ...
    cmd_buf: Datum<Datum<DeviceBuffer<Vec<vk::CommandBuffer>>>>,
