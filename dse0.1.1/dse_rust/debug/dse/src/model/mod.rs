    ...
    impl ModelE {
        ...
        pub fn update_global_transform_sync(
            ...
            for mi in datum_model.iter_mut() {
                if mi.attachment.switch_transform_update {
                    mi.attachment.switch_transform_update = false;
                    _node_index_array.push(mi.node_ref().index_self);
            ...
            if !_node_index_array.is_empty() {
                ...
                for i in _parent_count.._node_index_array.len() {
                    TransformD::update_global(
                        datum_trans,
                        get!(datum_model.vec_ref(),_node_index_array[i] as usize) 
                            .node_ref()
                            .index_parent,
                        _node_index_array[i],
                    )
