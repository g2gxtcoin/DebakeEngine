...
//use it when LogCodeD need to specifying code part
pub enum LogCodePart {
    Default,
    Type,
    Condition,
    Line,
...
impl LogCodeD {
    ...
    pub fn encode(&mut self, uin: u128, code_part: LogCodePart) -> Self {
        match code_part {
            LogCodePart::Type => {
                return Self {
                    data: unsafe { self.data | uin * 0x0000_00000000_0000_0000_0000_0000_0001 },
                }
            }
            LogCodePart::Condition => {
                return Self {
                    data: unsafe { self.data | uin * 0x0000_00000000_0000_0000_0000_0001_0000 },
                }
            }
            LogCodePart::Line => {
                return Self {
                    data: unsafe { self.data | uin * 0x0000_00000000_0000_0000_0001_0000_0000 },
                }
            }
            LogCodePart::Id => {
                return Self {
                    data: unsafe { self.data | uin * 0x0000_00000000_0000_0001_0000_0000_0000 },
                }
            }
            LogCodePart::ParentId => {
                return Self {
                    data: unsafe { self.data | uin * 0x0000_00000000_0001_0000_0000_0000_0000 },
                }
            }
            LogCodePart::Time => {
                return Self {
                    data: unsafe { self.data | uin * 0x0000_00000001_0000_0000_0000_0000_0000 },
                }
            }
            LogCodePart::FileID => {
                return Self {
                    data: unsafe { self.data | uin * 0x0001_00000000_0000_0000_0000_0000_0000 },
                }
            }
            LogCodePart::Default => {
                return Self {
    ...
    pub fn decode(&self, code_part: LogCodePart) -> u128 {
        unsafe {
            match code_part {
                LogCodePart::Type => return self.data & 0x0000_00000000_0000_0000_0000_0000_ffff,
                LogCodePart::Condition => {
                    return self.data & 0x0000_00000000_0000_0000_0000_ffff_0000
                }
                LogCodePart::Line => return self.data & 0x0000_00000000_0000_0000_ffff_0000_0000,
                LogCodePart::Id => return self.data & 0x0000_00000000_0000_ffff_0000_0000_0000,
                LogCodePart::ParentId => {
                    return self.data & 0x0000_00000000_ffff_0000_0000_0000_0000
                }
                LogCodePart::Time => return self.data & 0x0000_ffffffff_0000_0000_0000_0000_0000,
                LogCodePart::FileID => return self.data & 0xffff_00000000_0000_0000_0000_0000_0000,
                LogCodePart::Default => {
    ...
    pub fn get_subcode(&self, code_part: LogCodePart) -> u16 {
        unsafe {
            match code_part {
                LogCodePart::Type => self.part[0],
                LogCodePart::Condition => self.part[1],
                LogCodePart::Line => self.part[2],
                LogCodePart::Id => self.part[3],
                LogCodePart::ParentId => self.part[4],
                LogCodePart::Time => todo!(),
                LogCodePart::FileID => self.part[7],
                LogCodePart::Default => todo!(),
            }
        }
    }
...
fn decode_into_str(uin: u128) -> String {
    ...
    return String::from(
        "type:".to_owned()
            + &code.get_subcode(LogCodePart::Type).to_string()
            + "|"
            + "condition:"
            + &code.get_subcode(LogCodePart::Condition).to_string()
            + "|"
            + "exeID:"
            + &code.get_subcode(LogCodePart::Id).to_string()
            + "|"
            + "datID:"
            + &code.get_subcode(LogCodePart::ParentId).to_string()
            + "|"
            + "time:"
            + &code.get_time_ms().to_string()
            + "|"
            + "fileID:"
            + &code.get_subcode(LogCodePart::FileID).to_string()
            + "|"
            + "line:"
            + &code.get_subcode(LogCodePart::Line).to_string()
            + "|",
    );
}
    ...
    unsafe {
        if LOG_OFFSET < MAX_LOGGER_BUFFER_LEN {
            LOGGER_BUFFER[LOG_OFFSET] = uin
                | LogCodeD::new()
                    .encode(get_log_time(), LogCodePart::Time)
                    .get_code();
            ...
            crate::send2logger_dev!(
                crate::log::code::TYPE_CORE_ERROR
                    ...
                    | crate::log::LogCodeD::new()
                        .encode(line!() as u128, LogCodePart::Line)
                        .get_code()
            );
        }
    }
...
///8-15 pos is send log time: unit:per micro second
///16-19 pos is line info
///20-23 pos is execute id
///24-27 pos is data id
///28-31 pos is log count
#[allow(dead_code)]
pub mod code {
    /************************************************************************************file***time******id**pid**lin**con**type */
