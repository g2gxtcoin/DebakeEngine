...
pub mod env {
    ...
    use crate::{
        dbg_dev, ext_api::graphic::env::{name, AshVkBaseD}, get, manager::{
            datum::env::Datum,
            execute::{
                env::TaskQueue,
    ...
    use super::{cfg::{self, env::{IMG2VIEW::{DEFAULT_COLOR, DEFAULT_DEPTH}, IMG_FORMAT::{DEFAULT_COLOR_IMG, DEFAULT_DEPTH_IMG}}}, pipeline::{
        self,
        env::{
            GraphicPipeLinePCO, GraphicPipeLinePSO, PCOTrait,
            PSOTrait, RenderPipelineD, RenderPipelineType,
        },
    }};
    ...
    impl PreTypeSurfaceIMG {
                ...
                PreTypeSurfaceIMG::None => Err(crate::log::sorry(
                    crate::log::code::TYPE_EXE_ERROR
                        ...
                        | crate::log::LogCodeD::new()
                            ...
                            .get_code()
                        | crate::log::LogCodeD::new()
                            .encode(u16::MAX as u128, crate::log::LogCodePart::Id)
                            .get_code(),
                )),
                PreTypeSurfaceIMG::DefaultDepth => {
                    return Ok(DEFAULT_DEPTH_IMG.clone())
                }
                PreTypeSurfaceIMG::DefaultColor => {
                    return Ok(DEFAULT_COLOR_IMG.clone())
                }
            }
        }
        pub fn get_img_view_info(&self) -> Result<vk::ImageViewCreateInfo, ()> {
            match self {
                ...
                PreTypeSurfaceIMG::DefaultDepth => {
                    return Ok(DEFAULT_DEPTH.clone())
                }
                PreTypeSurfaceIMG::DefaultColor => {
                    return Ok(DEFAULT_COLOR.clone())
                }
            }
        }
    ...
    impl RendererE {
                    ...
                    crate::send2logger_dev!(
                        crate::log::code::TYPE_EXE_ERROR
                            ...
                            | crate::log::LogCodeD::new()
                                ...
                                .get_code()
                            | crate::log::LogCodeD::new()
                                .encode(renderer_slice.id as u128, crate::log::LogCodePart::Id)
                                .get_code()
                    );
                }
            }
        ...
        pub fn build_gpu_properties(mut self, api_in: &mut AshVkBaseD) -> Self {
        ...
        pub fn build_specify_api_base2create_surface(mut self, api_in: &mut AshVkBaseD) -> Self {
            self.device = Option::Some(api_in.ash_device_clone().unwrap());
            self.renderer_attachment.device_queue_count = api_in.gpu_suitable_queue_count_currrent();

            self._create_surface(api_in);
            return self;
        }

        pub fn build_device_suitable_surface(self, api_in: &mut AshVkBaseD) -> Self {
        ...
        pub fn build_swap_buffer(mut self, api_in: &mut AshVkBaseD) -> Self {
        ...
        pub fn exe_fbo(
            ...
            datum_pipeline: &mut Datum<
                RenderPipelineD<GraphicPipeLinePSO, GraphicPipeLinePCO>,
            >,
            tin: &mut Datum<TaskQueue<RendererTask>>,
        ) {
            let mut _tasks = tin
        ...
        fn _create_surface(&mut self, api_in: &mut AshVkBaseD) {
        ...
        fn _callback_create_graphic_pipeline_pass(
            ...
            for pi in datum.iter_mut() {
                let _pass = unsafe {
                    renderer_slice.device.as_mut().unwrap().create_render_pass(
                        pi.as_ref()
                            .unwrap()
                            .pso_ref()
                            .renderpass_info_ref()
