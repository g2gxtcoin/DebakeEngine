...
pub mod env {
    ...
    use crate::{dev_stop, renderer::cfg};
        ...
        TComponents: PCOTrait + Any,
    {
        ...
        fn _valid_vertex_input_state(&mut self, gpu_properties: &vk::PhysicalDeviceProperties) {
            ...
            // 判断已设vad是否不为空
            if self.states.vertex_input_state_mut().is_some() {
                self.states
                    .vertex_input_state_mut()
                    .unwrap()
                    ...
                    .is_null()
                {
                    self.states
                        .vertex_input_state_mut()
                        .unwrap()
                        .p_vertex_attribute_descriptions = &cfg::env::PSO::DEFAULT_VAD_4X4_RGBA64F;
                }
            }
        }
